...
export default function Store() {
  ...
  const albums = [
    { id: 'a1', title: 'MIDNIGHT', type: 'Album', price: 14.99, cover: '/assets/images/midnight_cover.png', isNew: true },
    { id: 'a2', title: 'THE AWAKENING', type: 'Album', price: 14.99, cover: '/assets/images/midnight_cover.png' }
  ];
  ...
  return (
    <div className="page-container fade-in">
      ...
      {/* Hero Album - MIDNIGHT */}
      <div className="glass-panel" style={{ display: 'flex', gap: '3rem', marginTop: '2rem', alignItems: 'center', background: 'linear-gradient(135deg, rgba(20,20,20,0.8), rgba(60,40,20,0.6))' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 1fr) 2fr', gap: '3rem', marginTop: '2rem' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {albums.map(album => (
              <div key={album.id} style={{ display: 'flex', gap: '1rem', alignItems: 'center', background: 'rgba(0,0,0,0.4)', padding: '1rem', borderRadius: '8px', border: '1px solid rgba(255,204,0,0.1)', cursor: 'pointer', transition: 'var(--transition-smooth)' }} className="album-card">
                <img src={album.cover} alt={album.title} style={{ width: '80px', height: '80px', borderRadius: '8px', border: '1px solid var(--color-earth)' }} />
                <div style={{ flex: 1 }}>
                  <h4 style={{ color: 'var(--color-parchment)', fontSize: '1.1rem' }}>{album.title}</h4>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>{album.type} • ${album.price.toFixed(2)}</p>
                </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {singles.map((song, i) => (
              <div key={song.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem', borderBottom: '1px solid var(--color-earth)', transition: 'background 0.2s' }} className="track-row">
                ...
                <div style={{ color: 'var(--text-secondary)', width: '60px', textAlign: 'right', paddingRight: '1rem' }}>{song.duration}</div>
                <button className="btn btn-secondary" style={{ padding: '0.4rem 1rem', fontSize: '0.85rem' }}>${song.price.toFixed(2)}</button>
              </div>
            ))}
          </div>
        </div>

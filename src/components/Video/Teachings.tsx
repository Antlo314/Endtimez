...
export default function Teachings({ user }: { user: any, profile?: any }) {
  ...
  const renderVideoCard = (video: any, isPremium: boolean = false) => (
    <div key={video.id} className="glass-panel" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', border: isPremium ? '1px solid var(--gold)' : '1px solid var(--glass-border)' }}>
      ...
      <div style={{ padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.5rem', flex: 1 }}>
          {isPremium ? 'Exclusive cinematic experience from DarakiBar.' : 'Deep dive into the prophetic timeline and scriptural mysteries.'}
        </p>
      ...
      <div style={{ width: '100%', borderTop: '1px solid rgba(212, 175, 55, 0.3)', paddingTop: '4rem', marginTop: '2rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <p style={{ color: 'var(--color-parchment)', fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto', opacity: 0.8 }}>
            Unlock exclusive high-production content, cinematic deep dives, and the pinnacle of Endtimez truth from our Master Architect, DarakiBar.
          </p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '2rem', marginBottom: '4rem' }}>
          {premiumPopular.map(v => renderVideoCard(v, true))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '2rem', marginBottom: '4rem' }}>
          {premiumRecent.map(v => renderVideoCard(v, true))}
        </div>
      </div>
    </div>
  );
